...
GLvoid drawScene()												//--- 콜백 함수: 그리기 콜백 함수
{
	...
	//	block1.Translate(0, -0.5 * glm::sec(glm::radians(back.rotZ)) + 0.0125 * glm::sec(glm::radians(back.rotZ)), 0);
	//}
	block1.Translate(0, block1.transY, 0);
...
GLvoid Timer(int value) {
	...
	case 'Y':
		...
		break;
	case 'F': {
		//block1.transY += block1.speed / 100;
		//block2.transY += block1.speed / 100;
		//block3.transY += block1.speed / 100;
		if (grav >= -45 && grav <= 45) {
			block1.transY -= 0.01;
			block2.transY += block1.speed / 100;
			block3.transY += block1.speed / 100;
			if (block1.transY <= -0.5) {
				block1.transY += 0.01;
				if (block1.transX >= -5) {
					block1.transX -= 0.01;
				}
			...
			if (block2.transY <= -0.5 * glm::sec(glm::radians(back.rotZ)))
				block2.transY -= block1.speed / 100;
			if (block3.transY <= -0.5 * glm::sec(glm::radians(back.rotZ))) {
				block3.transY -= block1.speed / 100;
			}
		}
		else if (grav > 45 && grav < 135) {
			...
			block3.transY -= block1.speed / 100;
			if (block1.transY <= -0.5)
				block1.transY = -0.5 * glm::csc(glm::radians(back.rotZ)) + 0.0125 * glm::csc(glm::radians(back.rotZ));
			if (block2.transY <= -0.5 * glm::sec(glm::radians(back.rotZ)))
				block2.transY -= block2.speed / 100;
			if (block3.transY <= -0.5 * glm::sec(glm::radians(back.rotZ))) {
				block3.transY -= -0.5 * glm::csc(glm::radians(back.rotZ)) + 0.0125 * glm::csc(glm::radians(back.rotZ));
			}
			//block1.Translate(0, -0.5 * glm::csc(glm::radians(back.rotZ)) + 0.0125 * glm::csc(glm::radians(back.rotZ)), 0);
		}
		else if (grav >= 135 && grav <= 225) {
			if (block1.transY < 0.5 * glm::sec(glm::radians(back.rotZ)))
				block1.transY -= block1.speed / 100;
			if (block2.transY < 0.5 * glm::sec(glm::radians(back.rotZ)))
				block2.transY -= block2.speed / 100;
			if (block3.transY < 0.5 * glm::sec(glm::radians(back.rotZ))) {
				block3.transY -= block3.speed / 100;
			}
			//block1.Translate(0, 0.5 * glm::sec(glm::radians(back.rotZ)) - 0.0125 * glm::sec(glm::radians(back.rotZ)), 0);
		}
		else if (grav > 225 && grav < 315) {
		...
		glutTimerFunc(10, Timer, 'F');			// 낙하 타이머
		break;
	}
	default:
		glutTimerFunc(10, Timer, value);
...
GLvoid MouseMove(int x, int y) {
	if (Lclick) {
		...
		grav = (int)back.rotZ % 360;
		if (grav >= -45 && grav <= 45) {
			block1.speed = (-0.5 * glm::sec(glm::radians(back.rotZ))) / glm::abs(-0.5 * glm::sec(glm::radians(back.rotZ))); // 낙하 방향벡터
		}
		else if (grav > 45 && grav < 135) {
			block1.speed = (0.5 * glm::csc(glm::radians(back.rotZ))) / glm::abs(0.5 * glm::csc(glm::radians(back.rotZ)));
		}
		else if (grav >= 135 && grav <= 225) {
			block1.speed = (0.5 * glm::sec(glm::radians(back.rotZ))) / glm::abs(0.5 * glm::sec(glm::radians(back.rotZ)));
		}
		else if (grav > 225 && grav < 315) {
			block1.speed = (0.5 * glm::csc(glm::radians(back.rotZ))) / glm::abs(0.5 * glm::csc(glm::radians(back.rotZ)));
		}

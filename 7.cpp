...
GLuint shaderID;
GLint width, height;
GLuint VAO[7], VBO, rectVBO,EBO;
int head[4], shape[4];
bool sizeplus, inmoveleft, colorchange[2];
float sizecnt = 5.0f;
...
float vertexData[6][3][6] = {
	...
	-0.4,0.3,0.0,		0.3,0.3,0.3,
	-0.5,0.3,0.0,		0.3,0.3,0.3,
	-0.45,0.1,0.0,		0.3,0.3,0.3,

	0.4,-0.3,0.0,		0.2,0.6,0.6,
	0.5,-0.3,0.0,		0.6,0.2,0.6,
	0.45,-0.1,0.0,		0.6,0.6,0.2

};

float rectData[]{
	-0.5,-0.3,0,		0.8,0.8,0.8,
	0.5,-0.3,0,			0.8,0.8,0.8,
	0.5,0.3,0,			0.8,0.8,0.8,
	-0.5,0.3,0,			0.8,0.8,0.8
};
...
void main(int argc, char** argv)								//--- 윈도우 출력하고 콜백함수 설정
{
	...
	glutTimerFunc(100, TimerFuction, 4);
	glutTimerFunc(100, TimerFuction, 5);
	//glutMouseFunc(Mouse);
	...
	case 0:
		...
		case 0:
			...
			if ((vertexData[value][0][0] + vertexData[value][1][0]) / 2 <= -1) {
				for (int i = 0; i < 3; ++i) {
					vertexData[value][i][3] = urd(dre) / 10.0;
					vertexData[value][i][4] = urd(dre) / 10.0;
					vertexData[value][i][5] = urd(dre) / 10.0;
				}
				head[value] = 1;
				if (shape[value] == 0) {
					shape[value] = 1;
				...
				if (shape[value] == 0) {
					float centery = (vertexData[value][2][1] - vertexData[value][0][1]) / 3.0f + vertexData[value][0][1];
					float centerx = (vertexData[value][0][0] + vertexData[value][1][0]) / 2.0f;
					if (centerx >= rectData[0] && centerx <= rectData[6] && centery >= rectData[1] && centery <= rectData[19]) {
						for (int i = 0; i < 3; ++i) {
							vertexData[value][i][3] = urd(dre) / 10.0;
							vertexData[value][i][4] = urd(dre) / 10.0;
							vertexData[value][i][5] = urd(dre) / 10.0;
						}
						if (vertexData[value][0][0] <= rectData[6]) {
							head[value] = 2;
							shape[value] = 3;
							float w = vertexData[value][1][0] - vertexData[value][0][0];
							float h = vertexData[value][2][1] - vertexData[value][0][1];
			...
			if ((vertexData[value][0][0] + vertexData[value][1][0]) / 2 >= 1) {
				...
				if (shape[value] == 0) {
					...
					if (centerx >= rectData[0] && centerx <= rectData[6] && centery >= rectData[1] && centery <= rectData[19]) {
						...
						if (vertexData[value][0][0] <= rectData[0]) {
							head[value] = 0;
							shape[value] = 3;
							float w = vertexData[value][1][0] - vertexData[value][0][0];
							float h = vertexData[value][2][1] - vertexData[value][0][1];
		...
		case 4:
			if (inmoveleft) {
				if (colorchange[0]) {
					for (int i = 0; i < 3; ++i) {
						for (int j = 3; j < 6; ++j) {
							vertexData[value][i][j] = urd(dre) / 10.0;
						}
					}
					colorchange[0] = false;
				}
				for (int i = 0; i < 3; ++i) {
					vertexData[value][i][0] -= 0.02;
				}
				if (vertexData[value][1][0] >= rectData[6]) {
					inmoveleft = true;
					colorchange[0] = true;
					colorchange[1] = true;
				}
